...
<script>
  layui.use('table', function(){
    ...
    table.on('tool(J_list)', function(obj){
      var data = obj.data;
      if(obj.event === 'del'){
        layer.confirm('确定删除该数据集吗', function(index){
          $.post('/datacode/data/del', {
            id: data.id
          }, function(args){
            console.info(args)
            if ('' ==args.result) {
        ...
        var index = layer.open({
          ...
          ,shade: 0
          ,maxmin: true
          ,content: '/datacode/data/editPage?id=' + data.id
          ,btn: ['提交', '关闭']
          ,yes: function(){
            window.frames[0].document.getElementById("J_info_submit_btn").click();
          }
    ...
    $('#J_edit_introduce').click(function(){
      $.post('/datacode/data/introduce/edit', {
